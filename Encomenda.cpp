...
int main(){
    ...
    while(true){
        ...
        if(existe==false){
            // procurar a 1º posição livre da matriz modelo_qtd
            for(int i=0;i<4;i++){
                if(modelo_qtd[i][0]==0){
        ...
        if(detalhe>=4){
            ...
            exportarCSV ();
            cout<<"VEncomenda exportada com sucesso!";

            gotoxy(0,15);
            cout<<"Para nova encomenda digite sS ou outra tecla seguida de Enter para Terminar:";
            cin>>continuar;
...
void exportarCSV (){
    cout<<"Exportar encomenda para ficehiro  CSV acrescentando à informação já existente.";
    ofstream ficheiro("Encomendas.csv", ios::app);
    for(int i = 0; i < 4; i++){
        if(modelo_qtd[i][0] != 0){
            ficheiro<<nenc<<';'<<modelo_qtd[i][0]<<';'<<modelos[modelo_qtd[i][0]-1]<<';'
                    <<precos[modelo_qtd[i][0]-1]<<';'
                    <<modelo_qtd[i][1]<<modelo_qtd[i][1]*precos[modelo_qtd[i][0]-1]<<endl;
        }
    }
